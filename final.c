...
char initial_map[18][32]; // Declare a separate array to store the initial map
...
void copyInitialMap()
{
    ...
    for (i = 0; i < 18; i++)
    {
        strcpy(initial_map[i], map[i]); // Copy each row of the map to the initial_map array
    }
}
...
int main() {
    ...
    int numIterations = 2; // Number of iterations for the animation
    int iteration;
...
void displayMenu() {
	...
	printCenteredText("1. OBJECTIVES", width, height);
	printCenteredText("2. ABOUT PACMATH", width, height);
	printCenteredText("3. ABOUT THE CREATORS", width, height);
	printCenteredText("4. PLAY", width, height);
	printCenteredText("5. LEADERBOARD", width, height);
	printCenteredText("===================================================\n\n", width, height);
    ...
    do {
        ...
        switch (ans) {
            ...
            case 4:
				...
				height = 4;
				printCenteredText("Input : ", width, height);
				printCenteredText("=====================================================", width, height);
				char diffi;
			    int speedmod = 3;
			    scanf(" %c", &diffi);
